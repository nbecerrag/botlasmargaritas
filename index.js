...
async function procesarYGuardarDatosConversacion(respuestaGemini, mensajeUsuario, waId) {
    ...
    // Primero intentar capturar hora con expresión española explícita
    let horaMatch = mensajeUsuario.match(/(\d{1,2})(?:\s*:\s*(\d{2}))?\s*(?:de\s+la\s+)?(tarde|noche|mañana|madrugada)/i);

    // Si no encuentra con expresión española, buscar con am/pm
    if (!horaMatch) {
        horaMatch = mensajeUsuario.match(/(\d{1,2})(?:\s*:\s*(\d{2}))?\s*(am|pm|a\.m\.|p\.m\.)/i);
    }

    // Si no encuentra ninguno, buscar cualquier hora con formato HH:MM
    if (!horaMatch) {
        horaMatch = mensajeUsuario.match(/(\d{1,2}):(\d{2})/);
    }
    ...
    if (horaMatch && (mensajeLower.includes('tarde') || mensajeLower.includes('noche') ||
        ...
        console.log(`🕐 Hora detectada: ${hora}, minutos: ${minutos}, palabra clave: ${periodo || 'detectando...'}`);
        ...
        if (periodo.includes('pm')) {
            if (hora !== 12) {
                console.log(`   → Convirtiendo ${hora}PM: ${hora} + 12 = ${hora + 12}`);
                hora += 12;  // 1-11pm → 13-23, 12pm stays 12
            } else {
                console.log(`   → 12PM se mantiene como 12`);
            }
        } else if (periodo.includes('am') && hora === 12) {
            console.log(`   → Convirtiendo 12AM a 00:00`);
            hora = 0;  // 12am → 00:00
        }
